...
uint256 hashGenesisBlock("0x6f231324e9572b7fb659f2de2b2addd758f6a761469710db9d617c2b6cb9bb2f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Africoin: starting difficulty is 1 / 2^12
...
{
    // Africoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Africoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Africoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Africoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Africoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4d1bebb1d66c6124984c73044458000b68752509ea99190bcc191595f795aee7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1626376359;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084851417;

        if (fTestNet)
        {
            block.nTime    = 1626376331;
            block.nNonce   = 385447258;
        }
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Africoin: increase each by adding 2 to bitcoin's value.
...
void static AfricoinMiner(CWallet *pwallet)
{
    printf("AfricoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("africoin-miner");
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AfricoinMiner, pwallet));
}
